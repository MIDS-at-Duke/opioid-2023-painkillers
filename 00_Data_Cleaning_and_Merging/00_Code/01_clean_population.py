# ## Using the nhgis platform to get census data for the years 2003-2015

# For 2003 to 2007, we extracted the population for all counties from the 1970-2007 data hosted on the nhgis website.
...
# Read in the data
pop2003 = pd.read_csv("nhgis0001_ds231_2003_county.csv")
pop2004 = pd.read_csv("nhgis0001_ds231_2004_county.csv")
pop2005 = pd.read_csv("nhgis0001_ds231_2005_county.csv")
...
# Shape is same for all, columns matching
# merging 2003 and 2004
pop34 = pd.merge(pop2003, pop2004, on=["STATEFP", "COUNTYFP"])
...
# rename columns
pop34.columns = ["GISJOIN", "STATEFP", "COUNTYFP", "COUNTY", "STATE", "2003", "2004"]
pop34

pop34.value_counts()


# Merging 2005 now to 2003 and 2004
pop5included = pd.merge(pop34, pop2005, on=["STATEFP", "COUNTYFP"])

# only keep the columns we need
pop5included = pop5included[
    [
        "GISJOIN_x",
        "STATEFP",
        "COUNTYFP",
...
# now merge pop (2006 and 2007) with pop5included (2003,2004,2005)

popfinal = pd.merge(pop, pop5included, on=["GISJOIN"])
...
df2008 = pd.merge(
    df2008, popfinal, left_on=["STATEA", "COUNTYA"], right_on=["STATEFP", "COUNTYFP"]
)
...
data2010 = pd.merge(
    data2010, data2014, how="left", left_on=["GISJOIN"], right_on=["GISJOIN"]
)
...
#
# Here is the summary:
# 1. Complete data for `2003`, `2004`, `2005`, `2006`, `2007`, `2010`
...
final["2013"] = np.where(
    final["2013"] == 0, (final["2012"] + yearly_change).astype(int), final["2013"]
)
...
final["2014"] = np.where(
    final["2014"] == 0, (final["2013"] + yearly_change).astype(int), final["2014"]
)
...
final["2015"] = np.where(
    final["2015"] == 0, (final["2014"] + yearly_change).astype(int), final["2015"]
)
